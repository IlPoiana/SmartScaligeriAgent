...
(:functions
    (cost-of-move ?from - location ?to - location)  ; Define the function for move cost
    (total-cost)
    (threshold)
  )
  ...
  (:action move
    ...
    :effect  
    (when (< (total-cost) (threshold))
        (and 
      (not (at ?a ?from)) 
      (at ?a ?to) 
      (increase (total-cost) (cost-of-move ?from ?to))
       )  ; Increase total cost by the cost of moving from 'from' to 'to'
    )
  )
